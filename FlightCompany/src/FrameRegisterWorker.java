...
public class FrameRegisterWorker extends JFrame implements ActionListener {
    ...
    // with default values.
    public FrameRegisterWorker() {
        setTitle("Registration Form");//windows title
        ...
        title = new JLabel("Worker registration");//main title
    ...
    public boolean validandRegister() {//set all the fields of the new worker and if all valid send true and will use the new object,else we will print errors
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == sub) {//if you want to submit the details and add the worker
            if ((term.isSelected()) && validandRegister()) {//only if terms and fields valid go for it
                if( FrameMainMenu.db.addWorker(newWorker)) {// check if the worker already exist
                    JOptionPane.showMessageDialog(this, "Your worker added sucssefuly","Sucsses",JOptionPane.INFORMATION_MESSAGE);
                    this.setVisible(false);
                    try {
                        FrameWorkers.main();
                    } catch (Exception ex) {
    ...
    {
        FrameRegisterWorker f = new FrameRegisterWorker();
    }
