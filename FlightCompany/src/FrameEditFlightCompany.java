...
class FrameEditFlightCompany extends JFrame implements ActionListener{
    private static FlightCompany flightCompany;
    ...
    public FrameEditFlightCompany()
    {
        setTitle("Edit Flight Company");                                // title of the screen
        ...
        title = new JLabel("Edit Flight Company Form");             // title of the screen
        ...
        tname = new JTextField(flightCompany.getName());                // text area to add name of the flight company
        ...
        tcompanyNumber = new JTextField(String.valueOf(flightCompany.getCompanyNumber()));  // text area to add company number of the flight company
        tcompanyNumber.setFont(new Font("Arial", Font.PLAIN, 15));
        tcompanyNumber.setSize(190, 20);
        tcompanyNumber.setLocation(250, 150);
        tcompanyNumber.setEditable(true);
        ...
        tFoundingYear = new JTextField(String.valueOf(flightCompany.getFoundingYear()));    // text area to add founding year of the flight company
        tFoundingYear.setFont(new Font("Arial", Font.PLAIN, 15));
        tFoundingYear.setSize(190, 20);
        tFoundingYear.setLocation(250, 200);
        tFoundingYear.setEditable(true);
        c.add(tFoundingYear);
    ...
    public boolean checkInput()                     // check if all the fields are filled correctly
    {
        ...
        if (Integer.parseInt(tFoundingYear.getText())<0 || Integer.parseInt(tFoundingYear.getText())>2022)
        {
            res.setForeground(Color.red);
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        if (e.getSource() == sub) {             // if click submit check all the inputs and add them to the flight company
            if (term.isSelected())
            {
                if (checkInput())
                {
                    JOptionPane.showMessageDialog(this,"Edit flight company edited sucssefuly");
                    this.setVisible(false);
                    flightCompany.setName(tname.getText());
                    flightCompany.setCompanyNumber(Integer.parseInt(tcompanyNumber.getText()));
                    flightCompany.setFoundingYear(Integer.parseInt(tFoundingYear.getText()));
                    try {
                        FrameMainMenu.activateMainGui(false, flightCompany);
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
                }
    ...
    {
        FrameEditFlightCompany efc = new FrameEditFlightCompany();
    }

    public static void ActiveEditFlightCompany(FlightCompany f){ // start of the frame
        flightCompany = f;
        FrameEditFlightCompany.main();
    }
}
